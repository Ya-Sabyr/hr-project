...
from src.auth.dependencies import user_required
from src.vacancy.schemas import VacancyPublic
from src.vacancy.service import vacancy_service
from typing import List
from fastapi import APIRouter, UploadFile, File, Depends, HTTPException
from sqlalchemy.orm import Session
from src.resume.service import resume_service
...
async def get_accepted_vacancies(db: AsyncSession = Depends(get_db)):
...
async def get_accepted_vacancy_by_id(vacancy_id: int, db: AsyncSession = Depends(get_db)):
...
@router.post("/")
async def upload_resume(file: UploadFile = File(...), db: AsyncSession = Depends(get_db), user: User = Depends(user_required)):
...
@router.get("/")
async def get_resumes(db: AsyncSession = Depends(get_db), user: User = Depends(user_required)):
...
async def create_application(vacancy_id: int, resume_id: int, db: Session = Depends(get_db), user: User = Depends(user_required)):
...
async def get_applications(user: User = Depends(user_required), db: AsyncSession = Depends(get_db)):
...
async def get_applied_vacancies(user: User = Depends(user_required), db: AsyncSession = Depends(get_db)):
...
async def get_applied_vacancy_details(user: User = Depends(user_required), db: AsyncSession = Depends(get_db)):
