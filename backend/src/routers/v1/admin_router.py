...
from src.admin.services.hr_admin_service import hr_admin_service
from src.admin.services.user_admin_service import user_admin_service
from src.admin.schemas import HRInDBAdmin, UserInDBAdmin
from src.vacancy.service import vacancy_service
from src.vacancy.schemas import VacancyInDBBase, VacancyStatusUpdate
...
async def get_all_hrs(db: AsyncSession = Depends(get_db)):
...
async def get_pending_hrs(db: AsyncSession = Depends(get_db)):
    logging.info("[HR FETCH] Fetching pending HRs")
    return await hr_admin_service.get_pending_hrs(db)

@router.delete("/hrs/{hr_id}")
async def delete_hr(hr_id: int, db: AsyncSession = Depends(get_db)):
...
async def approve_hr(hr_id: int, db: AsyncSession = Depends(get_db)):
...
async def toggle_block_hr(hr_id: int, db: AsyncSession = Depends(get_db)):
...
async def change_vacancy_status(vacancy_id: int, status_data: VacancyStatusUpdate, db: AsyncSession = Depends(get_db)):
...
async def get_vacancies_under_review(db: AsyncSession = Depends(get_db)):
