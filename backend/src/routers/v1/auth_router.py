...
async def login(obj_in: Login, db: AsyncSession = Depends(get_db), hr_service: HRService = Depends(get_hr_service)):
    logging.info(f"[AUTH LOGIN] Attempting login for user: {obj_in.email}")
    
    access_token, refresh_token, user_type, user_id = await AuthService.authenticate_user(obj_in.email, obj_in.password, db)
...
async def register_user(obj_in: UserCreate, db: AsyncSession = Depends(get_db)):
    logging.info(f"[USER REGISTRATION] Registering new user: {obj_in.email}")
    access_token, refresh_token = await AuthService.register_user(obj_in, db)
    logging.info(f"✅ [USER REGISTERED] User {obj_in.email} registered successfully")
    return {"access_token": access_token, "token_type": "bearer", "refresh_token": refresh_token}

@router.post("/register/hr")
async def register_hr(obj_in: HRCreate, db: AsyncSession = Depends(get_db)):
    logging.info(f"[HR REGISTRATION] Registering new HR: {obj_in.email}")
    await AuthService.register_hr(obj_in, db)  # Register HR without issuing a token
...
async def refresh_token_route(request: Request):
    ...
    access_token, refresh_token = await AuthService.verify_refresh_token(refresh_token)
